package nightswordv2WU

// Package: CORSAIR NIGHTSWORD v2 WIRELESS SD
...
type Device struct {
	...
	SurfaceSelections        map[int]string
	mutex                    sync.Mutex
...
var (
	...
	cmdSurfaceSelection       = []byte{0x01, 0x3c, 0x00}
	cmdMotionSync             = []byte{0x01, 0x13, 0x01}
	cmdRippleControl          = []byte{0x01, 0x14, 0x01}
	bufferSize                = 512
...
func Init(vendorId, productId uint16, _, path string) *common.Device {
	...
	d := &Device{
		...
		keyAssignmentFile: "/database/key-assignments/nightswordv2.json",
	...
	d.setSurfaceSelection()    // Surface Selection
...
func (d *Device) createDevice() {
	d.instance = &common.Device{
		ProductType: common.ProductTypeNightswordV2WU,
		Product:     d.Product,
		Serial:      d.Serial,
		Firmware:    d.Firmware,
...
// setSurfaceSelection will set surface selection
func (d *Device) setSurfaceSelection() {
	if d.DeviceProfile == nil {
		return
	}

	if d.DeviceProfile.SurfaceSelection < 0 || d.DeviceProfile.SurfaceSelection > 5 {
		return
	}

	buf := make([]byte, 1)
	buf[0] = byte(d.DeviceProfile.SurfaceSelection)
	_, _ = d.transfer(cmdSurfaceSelection, buf)
}
...
func (d *Device) setRippleControl() {
	...
	buf := make([]byte, 1)
	buf[0] = byte(d.DeviceProfile.RippleControl)
	_, _ = d.transfer(cmdRippleControl, buf)
}
...
func (d *Device) setMotionSync() {
	...
	buf := make([]byte, 1)
	buf[0] = byte(d.DeviceProfile.MotionSync)
	_, _ = d.transfer(cmdMotionSync, buf)
}
...
func (d *Device) saveDeviceProfile() {
	...
	if d.DeviceProfile == nil {
		...
		deviceProfile.SurfaceSelection = 0
	} else {
		...
		deviceProfile.SurfaceSelection = d.DeviceProfile.SurfaceSelection
...
// setDeviceDPIColor will set device DPI colors
func (d *Device) setDeviceDPIColor() {
...
// UpdateSurfaceSelection will update surface selection
func (d *Device) UpdateSurfaceSelection(surfaceSelection int) uint8 {
	if d.DeviceProfile == nil {
		return 0
	}

	if surfaceSelection < 0 || surfaceSelection > 5 {
		return 0
	}
	if d.DeviceProfile.SurfaceSelection == surfaceSelection {
		return 0
	}

	d.DeviceProfile.SurfaceSelection = surfaceSelection
	d.saveDeviceProfile()
	d.setSurfaceSelection()
	return 1
}
	...
	go func() {
		...
		for {
			...
			default:
				...
				if data[3] == 0x02 {
					d.triggerKeyAssignment(binary.LittleEndian.Uint16(data[4:6]))
				}
			}
		}
	}()
