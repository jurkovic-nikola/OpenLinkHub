...
var (
	...
	closeEndpoint        = []byte{0x05, 0x01, 0x01}
	cmdActivateButtons   = []byte{0x06, 0x01, 0x08}
...
func Init(vendorId, productId uint16, key string) *Device {
	...
	d.activateButtons()    // Activate buttons
	logger.Log(logger.Fields{"serial": d.Serial, "product": d.Product}).Info("Device successfully initialized")
	return d
}

// activateButtons will activate mouse buttons
func (d *Device) activateButtons() {
	_, err := d.transfer(cmdOpenWriteEndpoint, nil, "activateButtons")
	if err != nil {
	...
	_, err = d.transfer(cmdActivateButtons, buf, "activateButtons")
	if err != nil {
		logger.Log(logger.Fields{"error": err, "vendorId": d.VendorId}).Error("Unable to set dpi")
	}

	_, err = d.transfer(closeEndpoint, nil, "activateButtons")
	if err != nil {
		logger.Log(logger.Fields{"error": err, "vendorId": d.VendorId}).Error("Unable to set dpi")
	}
}
